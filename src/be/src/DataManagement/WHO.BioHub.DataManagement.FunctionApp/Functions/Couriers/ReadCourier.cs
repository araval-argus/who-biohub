...
public class ReadCourier
{
    private readonly ICouriersController _controller;

    public ReadCourier(ICouriersController controller)
    {
        _controller = controller;
    }

    [FunctionName(nameof(ReadCourier))]
    [OpenApiParameter(
        name: "id",
        Required = true,
    ...
    [OpenApiResponseWithBody(
        ...
        bodyType: typeof(ReadCourierQueryResponse),
    ...
    public async Task<IActionResult> ReadAsync(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "couriers/{id:Guid}")] HttpRequest req,
        ILogger log,
        Guid id,
        CancellationToken funcCancellationToken)
    {
        try
        {
            log.LogInformation("Read Courier function (HttpTrigger) invoked.");
