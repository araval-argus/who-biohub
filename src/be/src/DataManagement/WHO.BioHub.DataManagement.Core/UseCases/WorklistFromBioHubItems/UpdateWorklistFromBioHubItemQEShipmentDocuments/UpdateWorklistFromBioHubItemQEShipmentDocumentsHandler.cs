...
public interface IUpdateWorklistFromBioHubItemQEShipmentDocumentsHandler
{
    Task<Either<UpdateWorklistFromBioHubItemQEShipmentDocumentsCommandResponse, Errors>> Handle(UpdateWorklistFromBioHubItemQEShipmentDocumentsCommand command, CancellationToken cancellationToken);
}

public class UpdateWorklistFromBioHubItemQEShipmentDocumentsHandler : IUpdateWorklistFromBioHubItemQEShipmentDocumentsHandler
{
    private readonly ILogger<UpdateWorklistFromBioHubItemQEShipmentDocumentsHandler> _logger;
    private readonly UpdateWorklistFromBioHubItemQEShipmentDocumentsCommandValidator _validator;
    private readonly IUpdateWorklistFromBioHubItemQEShipmentDocumentsMapper _mapper;
    ...
    public UpdateWorklistFromBioHubItemQEShipmentDocumentsHandler(
        ILogger<UpdateWorklistFromBioHubItemQEShipmentDocumentsHandler> logger,
        UpdateWorklistFromBioHubItemQEShipmentDocumentsCommandValidator validator,
        IUpdateWorklistFromBioHubItemQEShipmentDocumentsMapper mapper,
    ...
    public async Task<Either<UpdateWorklistFromBioHubItemQEShipmentDocumentsCommandResponse, Errors>> Handle(
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (worklistfrombiohubitem.IsPast == true && !command.UserPermissions.Contains(PermissionNames.CanSubmitQESMTA2ShipmentDocumentsPast))
            {
                return new(new Errors(ErrorType.Unauthorized, $"Unauthorized"));
            }

            if (worklistfrombiohubitem.IsPast != true && !command.UserPermissions.Contains(PermissionNames.CanSubmitQESMTA2ShipmentDocuments))
            {
                return new(new Errors(ErrorType.Unauthorized, $"Unauthorized"));
            }
            ...
            await _worklistFromBioHubEngine.UpdateQEShipmentDocuments(worklistfrombiohubitem, moveToNextStatusCommand, transaction, cancellationToken);


            await transaction.CommitAsync();
            await transaction.DisposeAsync();

            return new(new UpdateWorklistFromBioHubItemQEShipmentDocumentsCommandResponse(worklistfrombiohubitem.Id));
    ...
    private MoveToNextStatusFromBioHubEngineCommand PrepareMoveToNextStatusCommand(UpdateWorklistFromBioHubItemQEShipmentDocumentsCommand command)
    {
