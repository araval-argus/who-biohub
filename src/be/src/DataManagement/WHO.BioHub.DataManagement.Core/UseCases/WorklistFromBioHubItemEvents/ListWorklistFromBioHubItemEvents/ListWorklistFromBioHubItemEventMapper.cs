...
namespace WHO.BioHub.DataManagement.Core.UseCases.WorklistFromBioHubItems.ListWorklistFromBioHubEventItem;

public interface IListWorklistFromBioHubItemEventMapper
{
    List<WorklistTimeline> Map(WorklistFromBioHubItem entity);
}

public class ListWorklistFromBioHubItemEventMapper : IListWorklistFromBioHubItemEventMapper
{

    public List<WorklistTimeline> Map(WorklistFromBioHubItem entity)
    {
    ...
    private List<WorklistTimelineEventsDayDto> GetList(WorklistFromBioHubItem entity, BookingForm? bookingForm = null)
    {
        List<WorklistTimelineEventsDayDto> list = new List<WorklistTimelineEventsDayDto>();      

        DateTime shipmentRequestDate = entity.WorklistFromBioHubHistoryItems.Any() ? (entity.WorklistFromBioHubHistoryItems.OrderBy(x => x.OperationDate).FirstOrDefault(x => x.Status == WorklistFromBioHubStatus.SubmitAnnex2OfSMTA2)?.OperationDate).GetValueOrDefault() : entity.OperationDate.GetValueOrDefault();
        ...
        DateTime? approvedAnnex2Date = GetApprovedEventDate(entity, WorklistFromBioHubStatus.WaitingForAnnex2OfSMTA2SECsApproval);
        ...
        DateTime? approvedBookingFormDate = GetApprovedEventDate(entity, WorklistFromBioHubStatus.WaitForBookingFormSMTA2OPSsApproval);
        ...
        var submitAnnex2Dates = GetMinorEventDates(entity, WorklistFromBioHubStatus.SubmitAnnex2OfSMTA2, true);
        ...
        var rejectAnnex2Dates = GetMinorEventDates(entity, WorklistFromBioHubStatus.WaitingForAnnex2OfSMTA2SECsApproval, false);
        ...
        DateTime? approvedBiosafetyChecklistDate = GetApprovedEventDate(entity, WorklistFromBioHubStatus.WaitForBiosafetyChecklistFormSMTA2BSFsApproval);

        if (approvedBiosafetyChecklistDate != null)
        {
            AddOrUpdateTimelineEventsDays(list, WorklistEventNames.ApprovedBiosafetyChecklist, (approvedBiosafetyChecklistDate.GetValueOrDefault().Date - shipmentRequestDate.Date).Days, approvedBiosafetyChecklistDate.GetValueOrDefault(), WorklistEventStageNames.PreShipment, WorklistEventType.Milestone);
        }

        var submitBiosafetyChecklistDates = GetMinorEventDates(entity, WorklistFromBioHubStatus.SubmitBiosafetyChecklistFormOfSMTA2, true);

        if (submitBiosafetyChecklistDates != null && submitBiosafetyChecklistDates.Any())
        {
            var firstSubmit = submitBiosafetyChecklistDates.OrderBy(x => x.Date).FirstOrDefault();
            AddOrUpdateTimelineEventsDays(list, WorklistEventNames.SubmitBiosafetyChecklist, (firstSubmit.Date - shipmentRequestDate.Date).Days, firstSubmit, WorklistEventStageNames.PreShipment, WorklistEventType.Minor);

            foreach (var submitBiosafetyChecklistDate in submitBiosafetyChecklistDates)
            {
                if (firstSubmit != submitBiosafetyChecklistDate)
                {
                    AddOrUpdateTimelineEventsDays(list, WorklistEventNames.ReSubmitBiosafetyChecklist, (submitBiosafetyChecklistDate.Date - shipmentRequestDate.Date).Days, submitBiosafetyChecklistDate, WorklistEventStageNames.PreShipment, WorklistEventType.Minor);
                }
            }
        }

        var rejectBiosafetyChecklistDates = GetMinorEventDates(entity, WorklistFromBioHubStatus.WaitForBiosafetyChecklistFormSMTA2BSFsApproval, false);

        if (rejectBiosafetyChecklistDates != null && rejectBiosafetyChecklistDates.Any())
        {
            foreach (var rejectBiosafetyChecklistDate in rejectBiosafetyChecklistDates)
            {
                AddOrUpdateTimelineEventsDays(list, WorklistEventNames.ReturnedBiosafetyChecklist, (rejectBiosafetyChecklistDate.Date - shipmentRequestDate.Date).Days, rejectBiosafetyChecklistDate, WorklistEventStageNames.PreShipment, WorklistEventType.Minor);
            }
        }


        var submitBookingFormDates = GetMinorEventDates(entity, WorklistFromBioHubStatus.SubmitBookingFormOfSMTA2, true);
        ...
        var rejectBookingFormDates = GetMinorEventDates(entity, WorklistFromBioHubStatus.WaitForBookingFormSMTA2OPSsApproval, false);
        ...
        DateTime? pickupCompletedDate = GetApprovedEventDate(entity, WorklistFromBioHubStatus.WaitForPickUpCompleted);
        ...
        DateTime? deliveryCompletedDate = GetApprovedEventDate(entity, WorklistFromBioHubStatus.WaitForDeliveryCompleted);
        ...
        DateTime? shipmentCompletedDate = entity.Status == WorklistFromBioHubStatus.ShipmentCompleted ? entity.OperationDate.GetValueOrDefault().Date : null;
    ...
    private DateTime? GetApprovedEventDate(WorklistFromBioHubItem entity, WorklistFromBioHubStatus previousStatus)
    {
        ...
        {
            if (entity.WorklistFromBioHubHistoryItems == null)
            {
                return null;
            }

            var historyEntity = entity.WorklistFromBioHubHistoryItems.Where(x => x.PreviousStatus == previousStatus && x.LastSubmissionApproved == true).FirstOrDefault();
    ...
    private List<DateTime> GetMinorEventDates(WorklistFromBioHubItem entity, WorklistFromBioHubStatus previousStatus, bool lastSubmissionApproved)
    {
        ...
        if (entity.WorklistFromBioHubHistoryItems != null && entity.WorklistFromBioHubHistoryItems.Any())
        {
            var historyEntityList = entity.WorklistFromBioHubHistoryItems.Where(x => x.PreviousStatus == previousStatus && x.LastSubmissionApproved == lastSubmissionApproved).ToList();
