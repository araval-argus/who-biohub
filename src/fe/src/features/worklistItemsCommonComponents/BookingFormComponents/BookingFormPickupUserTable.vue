...
export default class BookingFormPickupUserTable extends Vue {
  ...
  get bookingFormPickupUserGridItems(): Array<WorklistItemUserGridItem> {
    // const bookingForms = WorklistToBioHubItemModule.BookingForms;
    // let bookingFormPickupUsers = new Array<WorklistItemUser>();
    // if (bookingForms !== undefined) {
    //   const bookingForm = bookingForms.find((x) => x.Id === this.bookingFormId);
    //   if (bookingForm !== undefined) {
    //     bookingFormPickupUsers = bookingForm.BookingFormPickupUsers;
    //   }
    // }

    let bookingFormPickupUsers = this.model.BookingFormPickupUsers;

    if (bookingFormPickupUsers.length == 0)
      return new Array<WorklistItemUserGridItem>();

    return bookingFormPickupUsers.map((l) => {
      return {
        Id: l.Id,
        UserId: l.UserId,
        UserName: l.UserName,
  ...
  executeDelete(item: WorklistItemUserGridItem): void {
    this.$emit("removeBookingFormPickupUser", item.Id);
