<template>
  <v-container v-if="canApproveOrReject" fluid>
    ...
    >
      <v-row v-if="saveVisible">
        <v-spacer></v-spacer>
        <v-col cols="12" class="text-right">
          <v-btn color="success" class="mr-4" @click="onSave"> Send </v-btn>
        </v-col>
      </v-row>
    </UserRequestFormApprove>
  </v-container>
...
export default class UserRequestsPageApprove extends Vue {
  ...
  get canApproveOrReject(): boolean {
    return hasPermission(PermissionNames.CanApproveOrRejectUserRequest);
  }
  ...
  get saveVisible(): boolean {
    return (
      this.userRequest.IsConfirmed == true &&
      this.userRequest.ConfirmationDate != null &&
      this.userRequest.ConfirmationDate != undefined &&
      (this.userRequest.Status == UserRegistrationStatus.Approved ||
        this.userRequest.Status == UserRegistrationStatus.Rejected)
    );
  }
  ...
  async onSave(): Promise<void> {
    ...
    await UserRequestModule.ApproveOrRejectUserRequest()
