...
import { ShipmentMaterialGridItem } from "@/models/ShipmentMaterialGridItem";
...
export default class ShipmentMaterialsTable extends Vue {
  ...
  readonly bioHubFacilityId: string;

  get materialGridItems(): Array<ShipmentMaterialGridItem> {
    const materials = ShipmentModule.ShipmentMaterials;
    if (!materials) return new Array<ShipmentMaterialGridItem>();
    ...
    return materials.map((l) => {
      ...
      return {
        ...
        ShipmentMaterialCondition: l.ShipmentMaterialCondition,
      };
    });
  }
  ...
  selected(item: ShipmentMaterialGridItem): void {
    let destinationStatusRoute = "";

    if (
      hasPermission(PermissionNames.CanEditMaterial) &&
      (item.ShipmentMaterialCondition == ShipmentMaterialCondition.Intact ||
        hasPermission(PermissionNames.CanEditMaterialShipmentInformation))
    ) {
      destinationStatusRoute = "material-edit";

      if (this.routingNamePrefix == "biohubfacilityarea-") {
        if (
          this.bioHubFacilityId != "" &&
          this.bioHubFacilityId !=
            getUserBioHubFacilityOrLaboratoryOrCourierId(undefined)
        ) {
          destinationStatusRoute = "material-details-bmepp";
        }
      }
    } else {
      destinationStatusRoute = "material-details";
    }

    if (this.routingNamePrefix == "laboratoryarea-") {
      if (
        item.ProviderId ==
        getUserBioHubFacilityOrLaboratoryOrCourierId(undefined)
      ) {
        destinationStatusRoute = destinationStatusRoute + "-bmepp";
      } else {
        destinationStatusRoute = destinationStatusRoute + "-bmepp-catalogue";
      }
    }
    ...
    if (item.Status == MaterialStatus.WaitingForBioHubFacilityCompletion) {
      ...
      if (hasPermission(PermissionNames.CanApproveBioHubFacilityCompletion)) {
        ...
        if (
          (this.routingNamePrefix == "biohubfacilityarea-" &&
            this.bioHubFacilityId != "" &&
            this.bioHubFacilityId !=
              getUserBioHubFacilityOrLaboratoryOrCourierId(undefined)) ||
          (item.ShipmentMaterialCondition ==
            ShipmentMaterialCondition.Damaged &&
            !hasPermission(PermissionNames.CanEditMaterialShipmentInformation))
        ) {
