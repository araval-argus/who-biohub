...
import { ShipmentDocumentOperationType } from "@/models/WorklistToBioHubItem";
...
export default class ShipmentDocumentsForm extends Vue {
  ...
  async reloadData() {
    await WorklistToBioHubItemModule.ReloadShipmentDocuments(this.model.Id);
  }

  isTypePresent(type: DocumentFileType): boolean {
    const elem: ShipmentDocument | undefined =
      WorklistToBioHubItemModule.ShipmentDocuments.find(
        (x) => x.FileType == type
      );
  ...
  get shipmentDocuments(): Array<ShipmentDocument> {
    return this.model.ShipmentDocuments;
  }
  ...
  async deleteShipmentDocument(item: ShipmentDocument): Promise<void> {
    ...
    await WorklistToBioHubItemModule.UpdateWorklistToBioHubItemShipmentDocuments();
  ...
  async saveNewFile(file: Array<FormPopupItem>) {
    ...
    await WorklistToBioHubItemModule.UpdateWorklistToBioHubItemShipmentDocuments();
    WorklistToBioHubItemModule.SET_WORKLISTTOBIOHUBITEM_DOCUMENT(undefined);
    await this.reloadData();
