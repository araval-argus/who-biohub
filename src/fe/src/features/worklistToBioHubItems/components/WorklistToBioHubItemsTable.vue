    ...
    <v-card class="mb-5">
      ...
      <v-card-text>
        ...
        <v-data-table
          v-else
          :headers="headers"
          :items="worklistToBioHubItemGridItems"
          :search="search"
          :sort-by.sync="sortBy"
...
export default class WorklistToBioHubItemsTable extends Vue {
  ...
  get worklistToBioHubItemGridItems(): Array<WorklistToBioHubItemGridItem> {
    const worklistToBioHubItems =
      WorklistToBioHubItemModule.WorklistToBioHubItems;

    if (!worklistToBioHubItems)
      return new Array<WorklistToBioHubItemGridItem>();

    return worklistToBioHubItems.map((l) => {
      return {
        Id: l.Id,
  ...
  selected(item: WorklistToBioHubItemGridItem): void {
    const lab: WorklistToBioHubItem = this.getWorklistToBioHubItem(item.Id);
    this.$emit("selected", lab);
  }

  getWorklistToBioHubItem(id: string): WorklistToBioHubItem {
    const lab: WorklistToBioHubItem | undefined =
      WorklistToBioHubItemModule.WorklistToBioHubItems.find((x) => x.Id == id);
    if (lab === undefined)
      throw {
        message: `Unexpected undefined for WorklistToBioHubItem with id ${id}`,
      };
    return lab;
  }

  deleteItem(item: WorklistToBioHubItemGridItem): void {
