...
export default class ShippingMaterialsForm extends Vue {
  ...
  readonly shippingInformations: Array<MaterialShippingInformation>;
  ...
  get MaterialShippingInformationInfo(): any {
    const materialShippingInformationInfo = this.shippingInformations
      .filter((s) => {
        return s.Id == this.model.MaterialShippingInformationId;
      })
      .map((m) => {
        return {
          Quantity: m.Quantity,
    ...
    return materialShippingInformationInfo[0];
  }

  get Quantity(): number {
    return this.MaterialShippingInformationInfo.Quantity;
  }

  get Amount(): number {
    return this.MaterialShippingInformationInfo.Amount;
  }
