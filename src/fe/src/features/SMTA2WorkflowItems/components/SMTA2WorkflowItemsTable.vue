    ...
    <v-card class="mb-5">
      ...
      <v-card-text>
        ...
        <v-data-table
          v-else
          :headers="headers"
          :items="SMTA2WorkflowItemGridItems"
          :search="search"
          :sort-by.sync="sortBy"
...
export default class SMTA2WorkflowItemsTable extends Vue {
  ...
  get SMTA2WorkflowItemGridItems(): Array<SMTA2WorkflowItemGridItem> {
    const SMTA2WorkflowItems = SMTA2WorkflowItemModule.SMTA2WorkflowItems;

    if (!SMTA2WorkflowItems) return new Array<SMTA2WorkflowItemGridItem>();

    return SMTA2WorkflowItems.map((l) => {
      return {
        Id: l.Id,
  ...
  selected(item: SMTA2WorkflowItemGridItem): void {
    const lab: SMTA2WorkflowItem = this.getSMTA2WorkflowItem(item.Id);
    this.$emit("selected", lab);
  }

  getSMTA2WorkflowItem(id: string): SMTA2WorkflowItem {
    const lab: SMTA2WorkflowItem | undefined =
      SMTA2WorkflowItemModule.SMTA2WorkflowItems.find((x) => x.Id == id);
    if (lab === undefined)
      throw {
        message: `Unexpected undefined for SMTA2WorkflowItem with id ${id}`,
      };
    return lab;
  }

  deleteItem(item: SMTA2WorkflowItemGridItem): void {
