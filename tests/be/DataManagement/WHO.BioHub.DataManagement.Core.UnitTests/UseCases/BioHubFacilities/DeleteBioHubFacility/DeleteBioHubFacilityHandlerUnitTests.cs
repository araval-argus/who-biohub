...
namespace WHO.BioHub.DataManagement.Core.UnitTests.UseCases.BioHubFacilities.DeleteBioHubFacility;

public class DeleteBioHubFacilityHandlerUnitTests
{
    [Fact]
    public async Task If_resource_is_deleted_then_a_valid_response_is_returned()
    {
        // Arrange
        DeleteBioHubFacilityCommandValidator validatorMock = Substitute.For<DeleteBioHubFacilityCommandValidator>();
        ILogger<DeleteBioHubFacilityHandler> loggerMock = Substitute.For<ILogger<DeleteBioHubFacilityHandler>>();
        ...
        DeleteBioHubFacilityHandler handler = new(loggerMock, validatorMock, repositoryMock, userRepositoryMock);
        DeleteBioHubFacilityCommand cmd = new();
        ...
        repositoryMock
            ...
            .ReturnsForAnyArgs(
                Task.FromResult(biohubfacility));
        ...
        userRepositoryMock.DeleteUsersByBioHubFacility(biohubfacility.Id, cmd.OperationById, biohubfacility.DeletedOn.GetValueOrDefault(), cancellationToken, transactionMock).ReturnsForAnyArgs(Task.FromResult<Errors?>(null));

        // Act
        Either<DeleteBioHubFacilityCommandResponse, Errors> response = await handler.Handle(cmd, cancellationToken);
        ...
        {
            await validatorMock.ValidateAsync(Arg.Any<DeleteBioHubFacilityCommand>(), cancellationToken).Received(1);
            await repositoryMock.Update(biohubfacility, cancellationToken, transactionMock).Received(1);
        });
    }
