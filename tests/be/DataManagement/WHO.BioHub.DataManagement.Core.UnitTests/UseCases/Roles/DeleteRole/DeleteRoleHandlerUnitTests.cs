...
namespace WHO.BioHub.DataManagement.Core.UnitTests.UseCases.Roles.DeleteRole;

public class DeleteRoleHandlerUnitTests
{
    [Fact]
    public async Task If_resource_is_deleted_then_a_valid_response_is_returned()
    {
        // Arrange
        DeleteRoleCommandValidator validatorMock = Substitute.For<DeleteRoleCommandValidator>();
        ILogger<DeleteRoleHandler> loggerMock = Substitute.For<ILogger<DeleteRoleHandler>>();
        IRoleWriteRepository repositoryMock = Substitute.For<IRoleWriteRepository>();
        CancellationToken cancellationToken = default;

        DeleteRoleHandler handler = new(loggerMock, validatorMock, repositoryMock);
        DeleteRoleCommand cmd = new();
        ...
        // Act
        Either<DeleteRoleCommandResponse, Errors> response = await handler.Handle(cmd, cancellationToken);
        ...
        {
            await validatorMock.ValidateAsync(Arg.Any<DeleteRoleCommand>(), cancellationToken).Received(1);
